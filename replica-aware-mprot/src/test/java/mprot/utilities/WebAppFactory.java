...
public class WebAppFactory {
    ...
    public static Application createApplication(PiVersion piVersion){
        ...
        // frontend
        BindingPair frontendAskingHost = new BindingPair("frontend", "host");
        BindingPair frontendHostServer = new BindingPair("node", "host");
        testApp.addStaticBinding(frontendAskingHost, frontendHostServer);

        BindingPair frontendAskingConn = new BindingPair("frontend", "conn");
        BindingPair frontendConnServer = new BindingPair("backend", "conn");
        testApp.addStaticBinding(frontendAskingConn, frontendConnServer);

        // backend
        BindingPair backendAskingHost = new BindingPair("backend", "host");
        BindingPair backendHostServer = new BindingPair("node", "host");
        testApp.addStaticBinding(backendAskingHost, backendHostServer);

        BindingPair backendAskingDB = new BindingPair("backend", "db");
        BindingPair backendDBServer = new BindingPair("mongo", "db");
    ...
    public static Node createFrontend(){
        ...
        //phi: state -> list of states for fault handling
         //frontend do not offer any caps
        for (String state : frontend.getStates())
            frontendMP.addPhiEntry(state, new ArrayList<>());
    ...
    public static Node createBackend(){
        ...
        // phi: state -> list of states for fault handling
        for (String state : backend.getStates())
            backendMP.addPhiEntry(state, new ArrayList<>());
